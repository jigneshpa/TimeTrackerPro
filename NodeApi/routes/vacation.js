...
router.get('/requests', authenticate, async (req, res) => {
  try {
    ...
    const [requests] = await dbB.query(
...
router.post('/requests', authenticate, async (req, res) => {
  try {
    ...
    const [requests] = await dbB.query('SELECT * FROM vacation_requests WHERE id = ?', [result.insertId]);
...
router.post('/requests/cancel', authenticate, async (req, res) => {
  try {
    ...
    await dbB.query('UPDATE vacation_requests SET status = ? WHERE id = ?', ['cancelled', id]);
